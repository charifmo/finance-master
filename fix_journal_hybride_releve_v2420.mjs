/**
 * fix_journal_hybride_releve_v2420.mjs
 * v24.20 Journal Hybride Universel — 6 opérations
 *
 * Problème : quand un cycle passé (ex. "Mai 2026") est sélectionné dans le
 * Relevé (inline ou modal), le bilanJournal classique affichait une ligne
 * agrégée "Charges variables (Prorata Pilotage) = 14 000 DH".
 *
 * Fix : nouveau computed journalHybridePourReleve qui reconstruit les items
 * individuels depuis donneesAnnuelles pour N'IMPORTE quel cycle.
 * La table classique (bilanJournal) est retirée des deux endroits.
 *
 * Op 1 : Computed journalHybridePourReleve (JS — après journalHybride)
 * Op 2 : Inline Relevé — table hybride toujours visible, source = journalHybridePourReleve
 * Op 3 : Inline Relevé — supprimer table classique
 * Op 4 : Modal Relevé — table hybride toujours visible, source = journalHybridePourReleve
 * Op 5 : Modal Relevé — supprimer table classique
...
function check(label, needle) {
    if (!html.includes(needle)) {
        ...
        console.error(`   Preview: ${JSON.stringify(needle.slice(0, 150))}`);
        process.exit(1);
    }
}
...
// ── Op 1 : Computed journalHybridePourReleve (JS) ────────────────────────────
// Inséré juste après la fin de journalHybride (après la ligne return {...})
replace(
    'Op1 - journalHybridePourReleve computed',
    ...
    `                        return { entries, soldeAtterrissage: soldeProj };
                    ...
                    // v24.20 Journal Hybride Universel : items individuels pour tout cycle sélectionné
                    const journalHybridePourReleve = computed(() => {
                        calculationTick.value;
                        // Pas de cycle passé sélectionné → journal hybride standard
                        ...
                        // Solde d'ouverture du cycle : dernier soldeApres avant ce mois dans bilanJournal
                        ...
                        // Charges variables (items individuels)
                        ...
                        // Dépenses irrégulières du mois du cycle
...
// ── Op 2 : Inline Relevé — table hybride (source → journalHybridePourReleve) ──
replace(
    'Op2 - Inline table: source journalHybridePourReleve',
    ...
    `                                <!-- Journal Hybride Universel v24.20 : cycle actuel OU cycle passé sélectionné -->
...
// ── Op 3 : Inline Relevé — supprimer table classique ─────────────────────────
replace(
    'Op3 - Inline: supprimer table classique',
...
// ── Op 4 : Modal Relevé — table hybride (source → journalHybridePourReleve) ──
replace(
    'Op4 - Modal table: source journalHybridePourReleve',
    ...
    `                    <!-- Tableau : Mode Transactions — v24.20 Journal Hybride Universel -->
                <div v-if="!releveModeEvolution" class="flex-1 overflow-auto">
                    <!-- Journal Hybride Universel : cycle actuel OU cycle passé sélectionné -->
                    <table v-if="journalHybridePourReleve.entries.length > 0" class="w-full min-w-[400px] text-sm border-collapse">
                        <thead class="bg-gray-50 sticky top-0 z-10">
                            <tr>
                                <th class="p-2 text-left font-black text-[10px] uppercase tracking-widest text-gray-400 w-10">J.</th>
...
// ── Op 6 : Version + Changelog ────────────────────────────────────────────────
replace(
    'Op6 - Version bump → "24.20 Journal-Hybride-Universel"',
    `const CURRENT_VERSION = "24.10 Retro-KPI";`,
    `const CURRENT_VERSION = "24.20 Journal-Hybride-Universel";`
);
replace(
    'Op6b - Changelog entry v24.20',
    `const CHANGELOG = [`,
    `const CHANGELOG = [
        { version: "24.20 Journal-Hybride-Universel", date: "2026-05-29", changes: [
            "journalHybridePourReleve : items individuels pour tout cycle (actuel ou passe)",
            "Plus de ligne agregee Charges variables (Prorata Pilotage) dans le relevé cycle passé",
            "Inline et Modal Releve : une seule table hybride (suppression table classique bilanJournal)",
            "Footer footer : 'Solde de cloture' pour cycles passes, 'Atterrissage projeté' pour cycle actuel"
        ] },`
);

// ── Écriture ──────────────────────────────────────────────────────────────────
writeFileSync(FILE, html.replace(/\n/g, '\r\n'), 'utf8');
console.log(`\n🎉 v24.20 Journal-Hybride-Universel — ${opCount} opérations appliquées !`);
console.log(`📄 ${FILE}`);
